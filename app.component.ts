...
export class AppComponent implements OnInit, OnDestroy {
  ...
  readonly GRID_SIZE = 12; // 3 satır x 4 sütun = 12 alarm gösterilecek
  ...
  loadAlarms() {
    this.alarmService.getAlarms().subscribe((data: Alarm[]) => {
      // Sadece hatalı makineleri göster
      this.alarms = data.filter(alarm =>
        alarm.faultReason?.trim() !== '' &&
        alarm.faultTime?.trim() !== '' &&
        alarm.faultReason?.toLowerCase() !== 'yok' &&
        alarm.faultReason?.toLowerCase() !== '—' &&
        alarm.faultReason?.toLowerCase() !== '-' &&
        alarm.faultReason?.toLowerCase() !== 'none'
      );
